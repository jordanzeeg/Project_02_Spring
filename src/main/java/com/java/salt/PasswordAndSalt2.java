...
public class PasswordAndSalt2 {
	private static final Random RANDOM = new SecureRandom();
	private static final String ALPHABET = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
	private static final int ITERATIONS = 10000;
	private static final int KEY_LENGTH = 4;

	public String getresetkey(int length) {
		StringBuilder returnresetkey = new StringBuilder(length);

		for (int i = 0; i < length; i++) {
			returnresetkey.append(ALPHABET.charAt(RANDOM.nextInt(ALPHABET.length()))); // creating the salt
		}

		return new String(returnresetkey); // returning the salt as a string
	}

	public byte[] hash(char[] password, byte[] resetkey) {
		PBEKeySpec spec = new PBEKeySpec(password, resetkey, ITERATIONS, KEY_LENGTH);
		...
		try {
			...
			throw new AssertionError("Error while hashing a resetkey: " + e.getMessage(), e);
			// e.printStackTrace();
		} finally {
			spec.clearPassword();
		}
